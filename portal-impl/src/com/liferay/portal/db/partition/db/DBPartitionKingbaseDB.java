...
import com.liferay.portal.dao.db.KingbaseDB;
 ...
 */
public class DBPartitionKingbaseDB implements DBPartitionDB {
	...
	@Override
	  ...
	  public List<String> getCreateRulesSQL(String partitionName) throws
	  SQLException {
	  ...
	  try (Connection connection = DataAccess.getConnection()) { String sql =
	  StringBundler.concat( "select pg_catalog.pg_rewrite.rulename rulename, ",
	  ...
	  "pg_catalog.pg_class.oid where ", "pg_catalog.pg_class.relnamespace ='",
	  _defaultPartitionName,
	  "'::regnamespace and (pg_catalog.pg_rewrite.rulename like ",
	  "'delete_%' or pg_catalog.pg_rewrite.rulename like ", "'update_%')");
	  
	  
	  try (PreparedStatement preparedStatement = connection.prepareStatement(sql);
	  ResultSet resultSet = preparedStatement.executeQuery()) {
	  
	  while (resultSet.next()) { String ruleDefinition = StringUtil.toLowerCase(
	  resultSet.getString("ruledefinition"));
	  
	  String ruleName = resultSet.getString("rulename");
	  
	  String[] ruleTableColumn = getRuleTableColumn( ruleDefinition);
	  
	  if (!StringUtil.equals( ruleName, StringBundler.concat( "delete_",
	  ruleTableColumn[0], StringPool.UNDERLINE, ruleTableColumn[1])) &&
	  !StringUtil.equals( ruleName, StringBundler.concat( "update_",
	  ruleTableColumn[0], StringPool.UNDERLINE, ruleTableColumn[1]))) {
	  
	  continue; }
	  
	  rules.add( KingbaseDB.getCreateRulesSQL( StringBundler.concat( partitionName,
	  StringPool.PERIOD, ruleTableColumn[0]), ruleTableColumn[1])); } } }
	  
	  return rules; }
	 

	@Override
	public String getCreateTableSQL(Connection connection, String fromPartitionName, String toPartitionName,
			String fromTableName, String toTableName) throws SQLException {

		StringBundler sb = new StringBundler();

		sb.append(StringBundler.concat("create table if not exists ", toPartitionName, StringPool.PERIOD, toTableName,
				" (like ", fromPartitionName, StringPool.PERIOD, fromTableName,
		...
		for (String columnName : db.getPrimaryKeyColumnNames(connection, fromTableName)) {
		...
		for (IndexMetadata indexMetadata : db.getIndexMetadatas(connection, fromTableName, null, false)) {
			...
			sb.append(StringUtil.replace(indexMetadata.getCreateSQL(null), "on " + fromTableName,
					StringBundler.concat("on ", toPartitionName, StringPool.PERIOD, toTableName)));
		}
